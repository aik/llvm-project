; Tests both InstrEmitter::EmitMachineNode correctly sets predecessors and
; successors, AND runs the machine verifier.

; RUN: llc -verify-machineinstrs -print-after=finalize-isel -mtriple=i686-- < %s 2>&1 | FileCheck %s
...
; Check the second INLINEASM_BR target block is predecessed by the block with
; the second INLINEASM_BR.
